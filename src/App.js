...
const App = () => {
  ...
  useEffect(() => {
    if (isStart) {
      interval.current = setInterval(() => {
        // by 2 second isMove value change
  ...
  const checkConflict = () => {
    ...
    if (enemy !== null && character !== null) {
      // console.log(`>>>: checkConflict -> character.x`, character.x)
      // console.log(`>>>: checkConflict -> enemy.x`, enemy.x)
      let dis = Math.pow(enemy.x - character.x, 2) + Math.pow(enemy.y - character.y, 2)
      // console.log(`>>>: checkConflict -> dis`, dis)
      if (dis < 28000) {
        alert('Game Over!')
      ...
      {isStart ? (
        ...
        <div>
          <Button onClick={handleClickStartButton} className={classes.button}>
            Your Highest Record : {Math.floor(result)} seconds
            <br />
            Click to Start Game!
          </Button>
        </div>
